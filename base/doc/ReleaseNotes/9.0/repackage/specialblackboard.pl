...
# deals with very special cases in the org.cougaar.core.blackboard package
...
#	print "\nfile name is: ", $file;
	if (substr($file, -33) eq "BlackboardMetricsServiceImpl.java") {
	    $found++;
	    process_service($_);
	} elsif (substr($file, -30) eq "BlackboardServiceProvider.java") {
	    $found++;
	    process_service($_);
	} elsif (substr($file, -23) eq "StandardBlackboard.java") {
	    $found++;
	    process_service($_);
	}
    }
