...
# deals with very special cases in the org.cougaar.core.node package
...
#	print "\nfile name is: ", $file;
	if (substr($file, -9) eq "Node.java") {
	    $found++;
	    add_services($_);
	} elsif (substr($file, -31) eq "NodeMetricsServiceProvider.java") {
	    $found++;
	    add_services($_);
	} 
    }
