...
# deals with very special cases in the org.cougaar.core.service package
...
#	print "\nfile name is: ", $file;
	if (substr($file, -28) eq "MessageTransportService.java") {
	    $found++;
	    process_mts($_);
	} elsif (substr($file, -26) eq "MessageWatcherService.java") {
	    $found++;
	    process_mts($_);
	} elsif (substr($file, -17)eq "AlarmService.java") {
	    $found++;
	    process_alarm($_);
	} elsif (substr($file, -18)eq "DomainService.java") {
	    $found++;
	    process_domain($_);
	} elsif (substr($file, -23)eq "DemoControlService.java") {
	    $found++;
	    process_alarm($_);
	}
    }
